#include <climits>
#include <iostream>
#include <vector>

struct Node {
    Node() = default;
    Node(long long max, long long add) : max(max), add(add) {}

    long long max = LLONG_MIN;
    long long add = 0;
};

class SegmentTree {
public:
    explicit SegmentTree(size_t n) {
        while (size_ < n) {
            size_ *= 2;
        }
        tree_.assign(2 * size_ - 1, {});
    }

    void Build(const std::vector<long long>& nums) {
        Build(nums, 0, 0, size_);
    }

    long long GetMax(size_t l, size_t r) const  {
        return GetMax(l, r, 0, 0, size_);
    }

    void Add(size_t l, size_t r, long long val) {
        Add(l, r, val, 0, 0, size_);
    }
private:
    void Build(const std::vector<long long>& nums, size_t x, size_t lx, size_t rx) {
        if (rx == lx + 1) {
            if (lx < nums.size()) {
                tree_[x].max = nums[lx];
            }
        } else {
            const size_t mid = (lx + rx) / 2;
            const size_t left_child = LeftChild(x);
            const size_t right_child = RightChild(x);

            Build(nums, left_child, lx, mid);
            Build(nums, right_child, mid, rx);

            tree_[x].max = std::max(tree_[left_child].max, tree_[right_child].max);
        }
    }

    long long GetMax(size_t l, size_t r, size_t x, size_t lx, size_t rx) const {
        if (lx >= r || rx <= l) {
            return LLONG_MIN;
        }
        if (lx >= l && rx <= r) {
            return tree_[x].max;
        }

        const size_t mid = (lx + rx) / 2;
        const auto lhs = GetMax(l, r, LeftChild(x), lx, mid);
        const auto rhs = GetMax(l, r, RightChild(x), mid, rx);
        auto res = std::max(lhs, rhs);

        return res + tree_[x].add;
    }

    void Add(size_t l, size_t r, long long val, size_t x, size_t lx, size_t rx) {
        if (lx >= r || rx <= l) {
            return;
        }

        if (lx >= l && rx <= r) {
            tree_[x].add += val;
            tree_[x].max += val;
            return;
        }
        const size_t mid = (lx + rx) / 2;
        Add(l, r, val, LeftChild(x), lx, mid);
        Add(l, r, val, RightChild(x), mid, rx);
        tree_[x].max = std::max(tree_[LeftChild(x)].max, tree_[RightChild(x)].max) + tree_[x].add;
    }

    size_t LeftChild(size_t i) const {
        return 2 * i + 1;
    }

    size_t RightChild(size_t i) const {
        return 2 * i + 2;
    }

    size_t size_ = 1;
    std::vector<Node> tree_;
};

int main() {
    size_t n;
    std::cin >> n;

    std::vector<long long> nums(n);
    for (size_t i = 0; i < n; ++i) {
        std::cin >> nums[i];
    }

    SegmentTree tree(n);
    tree.Build(nums);

    int m;
    std::cin >> m;
    for (int i = 0; i < m; ++i) {
        char query_type;
        size_t l, r;
        std::cin >> query_type >> l >> r;
        if (query_type == 'm') {
            std::cout << tree.GetMax(l - 1, r) << ' ';
        } else if (query_type == 'a') {
            long long val;
            std::cin >> val;
            tree.Add(l - 1, r, val);
        }
    }
    return 0;
}
